...
// New creates a WorkQueue with a list of work to perform.
func New(items []string, f func(string, chan<- string), options ...Option) *WorkQueue {
	wq := &WorkQueue{
		checkpointInterval: defaultCheckpointInterval,
		numWorkers:         defaultNumWorkers,
		processItem:        f,
...
func (wq *WorkQueue) checkpoint() {
	for range time.NewTicker(wq.checkpointInterval).C {
		...
		util.Must(os.WriteFile(fname, []byte(sb.String()), 0600))
	}
}
